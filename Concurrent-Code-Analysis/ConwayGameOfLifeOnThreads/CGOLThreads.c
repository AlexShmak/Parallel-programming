    ...
    for (col = -1; col <= 1; col++) {
      ...
      if (row || col)
        if (curptr[i + row][j + col])
          count++;
      // we don't need to keep counting if the number is >3 (no change in
      // behaviour)
      if (count > 3) { // break nested loops
        break;
        break;
      }
    }
...
void *entry_function(void *t_id) {
  ...
  for (i = 0; i < 20; i++) {
    ...
    // use of the pthread_barrier_wait subroutine to synchronize multiple
    // threads
    // at a designated barrier. The calling thread is blocked until the required
    // number of threads have also called the subroutine, at which point the
    // barrier is reset to its most recent state. The constant
    // PTHREAD_BARRIER_SERIAL_THREAD is returned to one thread, while 0 is
    ...
    if (i == 0) {
      ...
      printf("-----------------------------\n");
      printf("   \"Set console dimensions at least %dx%d to have a full view "
             "of the board.\"",
             width, height + 4);
